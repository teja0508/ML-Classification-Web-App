...
def main():
    ...
    def load_data():
        data = pd.read_csv("C:\\Users\\Riyansika\\Desktop\\Heroku-Deployment-projects\\streamlit-ml-classification-web-app\\mushrooms.csv")
    ...
    if st.sidebar.checkbox("Show data shape", False):
        st.subheader("Mushroom Data Shape (Classification)")
        st.write(df.shape)
        
    if st.sidebar.checkbox("Correlation Plot(Seaborn)", False):
        st.subheader("Mushroom Correlation Plot(Seaborn) (Classification)")
        st.write(sns.heatmap(df.corr(),annot=True))
        st.pyplot()
    
    if st.sidebar.checkbox("Show Columns", False):
    ...
    if st.sidebar.checkbox("Show Value_Counts", False):
        st.subheader("Value_Counts")
        st.write(df.iloc[:,0].value_counts())
        
    if st.sidebar.checkbox("Data information", False):
